...
NSString *const FXFormNotificationFieldChanged = @"notificationFieldChanged"; //notification
...
NSString *const FXFormFieldDetailTitle = @"detail";
...
NSString *const FXFormFieldEditable = @"userInteractionEnabled";
...
static void FXFormPreprocessFieldDictionary(NSMutableDictionary *dictionary)
{
    ...
    //get editable status
    id defaultEditable = dictionary[FXFormFieldEditable];
    if (defaultEditable)
    {
        if ([valueClass isSubclassOfClass:[NSArray class]] && ![defaultEditable isKindOfClass:[NSArray class]])
        {
            //workaround for common mistake where type is collection, but default value is a single value
            defaultEditable = [valueClass arrayWithObject:defaultEditable];
        }
        else if ([valueClass isSubclassOfClass:[NSSet class]] && ![defaultEditable isKindOfClass:[NSSet class]])
        {
            //as above, but for NSSet
            defaultEditable = [valueClass setWithObject:defaultEditable];
        }
        else if ([valueClass isSubclassOfClass:[NSOrderedSet class]] && ![defaultEditable isKindOfClass:[NSOrderedSet class]])
        {
            //as above, but for NSOrderedSet
            defaultEditable = [valueClass orderedSetWithObject:defaultEditable];
        }
        dictionary[FXFormFieldEditable] = defaultEditable;
    }
...
{
    //don't distinguish between these, because we're always in edit mode
    // TODO: evaluate     FXFormFieldEditable
...
- (void)updateFieldValue
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:FXFormNotificationFieldChanged object:self.field];
}
